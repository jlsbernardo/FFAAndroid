...
class Demo extends DB
{
    ...
    protected $deletedModuleName = 'demo';
            ...
            if ($result->num_rows > 0) {
                ...
                foreach($rows as $row) {
                    ...
                    $demoRNAFields = array(
                        ...
                        'followup'      => !empty($row['followup']) && !is_null($row['followup']) ? $row['followup'] : NULL,
                        'marked_on_e'      => !empty($row['marked_on_e']) && !is_null($row['marked_on_e']) ? $row['marked_on_e'] : NULL,
                        'marked_on_f'      => !empty($row['marked_on_f']) && !is_null($row['marked_on_f']) ? $row['marked_on_f'] : NULL
                    );
    ...
    public function insertIntoStaging($data, $lastInserted = null)
    {
        ...
        foreach ($dataChunkeds as $dataChunked)
        {
            ...
            $insertQueryValue = [];
            foreach ($dataChunked as $demoRNAField) {
                if (!empty($demoRNAField['create_on']) && !is_null($demoRNAField['create_on']) || !empty($demoRNAField['update_on']) && !is_null($demoRNAField['update_on'])) {  
                    // if (date('Y-m-d') == convert_to_datetime($demoRNAField['create_on'], 'Y-m-d') || date('Y-m-d') == convert_to_datetime($demoRNAField['update_on'], 'Y-m-d')) {
                        $isInsertedStaging = $this->isInsertedStaging($ffaRecordStagingList, $demoRNAField['ffa_id']);
                        if (!$isInsertedStaging) {
                            
                            $strValues =  populate_insert_values(array_values($demoRNAField));
